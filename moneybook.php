...
while ($rows=mysqli_fetch_array($result1))
{
  ...
  if($bgcolor=='#cfffaa')
   ...
   $sql="select * from rrsv_student_registration where scl_reg_no='".$rows['scl_reg_no']."' and scl_session='".$rows['scl_session']."'";   
   $res=mysqli_query($myDB,$sql);
   while($row=mysqli_fetch_array($res)){
        $c=$row['scl_class'];
          
  $sql1="select sum(rate) as r from rrsv_copy where scl_class='".$row['scl_class']."'  and scl_session='".$rows['scl_session']."'";   
   $res1=mysqli_query($myDB,$sql1);
   while($obj=mysqli_fetch_array($res1)){
        $rate=$obj['r'];
 
          }
   }                

//echo $rows['total_monthly_fee']+$rows['add_total_cost']+$totalcarcost-$rate;
    /* Total Calculation */
...
$totalcardue                            =$totalcarcost-$rows['carfee']; /* Calcutation for Total amount due for car*/
 $add_total_cost                         =$rows['total_monthly_fee']+$rows['add_total_cost']+ $rows['copycost']+$totalcarcost-$rate; /* Total Gross amount with car New Admission*/
 $re_total_cost                          =$rows['total_monthly_fee']+$rows['re_total_cost']+ $rows['copycost']+$totalcarcos-$rate;  /* Total Gross amount with car Re Admission*/
$add_total_cost1                        =$rows['total_monthly_fee']+$rows['add_total_cost']+ $rows['copycost']-$rate; /* Total Gross amount with out car New Admission*/
$re_total_cost1                         =$rows['total_monthly_fee']+$rows['re_total_cost']+ $rows['copycost']-$rate;  /* Total Gross amount with out car Re Admission*/ 
